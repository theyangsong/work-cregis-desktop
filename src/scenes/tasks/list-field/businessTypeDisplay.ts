...
export const BUSINESS_TYPE_SECONDARY_LABEL_KEYS = {
  ...
  memberWalletRemittance: 'Wallet｜Payout',
} as const;
...
function resolveBusinessTypeSecondaryKey(transfer: TransferTypeRowValues): string {
  ...
  if (transfer.value === 'Remittance') {
    return BUSINESS_TYPE_SECONDARY_LABEL_KEYS.memberWalletRemittance;
  }
...
function findBusinessTypeCompositeSeparatorIndex(compositeKey: string): number {
  const fullWidth = compositeKey.indexOf('｜');
  const halfWidth = compositeKey.indexOf('|');
  if (fullWidth >= 0 && halfWidth >= 0) return Math.min(fullWidth, halfWidth);
...
export function splitBusinessTypeSecondaryKey(
  ...
  const separatorIndex = findBusinessTypeCompositeSeparatorIndex(compositeKey);
