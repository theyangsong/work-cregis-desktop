 ...
 * | R2 | E 自身带 catalog `eds-*` 根类，或 E 就是某 EDS 组件的 Vue DOM 根 | 该组件 + props |
 * | R3 | E 是排版文本叶子 | `Text` |
 * | R4 | E 带某 EDS 组件 CSS Module 的**具名 Figma 组件区域**类 | 该区域名 |
 * | R5 | 其余 | HTML 标签名（`Div` / `Span` / `Td`…） |
 *
 * ## 一层一名：禁止祖先借名 / 继承
 *
 * 每个 EDS 组件名**只出现在它自己的根节点上**。组件内部的普通容器
 * （ToolBar 的 `_functional_`、Popup 的 `_stage_`、Detail 的 `_itemTitle_`…）
 * 在 EDS 里就是 `<div>`，Figma 里也没有对应组件 —— 一律 R5 显示 HTML 标签。
 *
 * **【禁止】** 任何形式的祖先取名：`root.contains(element)`、沿 `parentElement`
 * 回溯最近具名层、按渲染归属继承。三者都会让父级与子级同名，进而属性 / 代码片段
 * 取到同一份参数 —— 这是本模块反复回归的根因。
 *
 * 「内层要不要组件名」只看 **它自己是不是 EDS 组件**（R2 覆盖任意 EDS 包组件根，
